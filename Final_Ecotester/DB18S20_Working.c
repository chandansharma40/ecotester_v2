...
uint8_t therm_reset()
{
	...
	THERM_INPUT_MODE(DQ);
	_delay_us(80);														// Should be 70 us
	//Store line value and wait until the completion of 480uS period
	i=(THERM_PIN & (1<<DQ));
	_delay_us(400);														// Should be 410 us
...
int16_t therm_read_temperature(uint8_t DQ_pass, int16_t *tes)
{
	...
	if ((temperature[8] == crc) && (crc != 0xFF))
	{
		...
		*tes = ceil(f_temp*10);
		...
		/*
		UART_putc(temperature[0]);
		UART_putc(temperature[1]);
		UART_putc(temperature[2]);
		UART_putc(temperature[3]);
		UART_putc(temperature[4]);
		UART_putc(temperature[5]);
		UART_putc(temperature[6]);
		UART_putc(temperature[7]);
		UART_putc(temperature[8]);
		UART_putc(crc);
		UART_putc(*tes>>8);
		UART_putc(*tes);
		*/
